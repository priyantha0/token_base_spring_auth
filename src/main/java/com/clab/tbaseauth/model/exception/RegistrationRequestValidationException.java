...
public class RegistrationRequestValidationException extends RuntimeException {

  private final transient BindingResult errors;

  public RegistrationRequestValidationException(BindingResult bindingResult) {
  ...
  private List<String> getValidationMessage(BindingResult bindingResult) {
    return bindingResult.getAllErrors().stream()
        .map(RegistrationRequestValidationException::getValidationMessage)
        .toList();
  }
  ...
  private static String getValidationMessage(ObjectError error) {
    ...
    return MessageFormat.format(
        "{0}.{1} {2}, but was {3} ", className, property, message, invalidValue);
  }
}
